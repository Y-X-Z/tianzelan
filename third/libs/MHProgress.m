...
//    UIView *view = [[UIView alloc] initWithFrame:CGRectMake((kScreenW - w) / 2, (kScreenH - h - correction) / 2 + correction, w, h)];
//    view.backgroundColor = [UIColor blackColor];
//    view.alpha = 0.6;
//    view.layer.masksToBounds = YES;
//    view.layer.cornerRadius = kCornerRadius;
//    [self addSubview:view];
    //UIImageView *view=[YX_myUI createImageViewFrame:CGRectMake((kScreenW - w) / 2, (kScreenH - h - correction) / 2 + correction, w, h) Image:[UIImage imageNamed:@"haitun"]];
    UIImageView *view=[[UIImageView alloc]initWithFrame:CGRectMake((kScreenW - w) / 2, (kScreenH - h - correction) / 2 + correction, w, h)];
    view.image=[UIImage imageNamed:@"haitun"];
    //view.backgroundColor = [UIColor blackColor];
    view.alpha = 0.6;
    view.layer.masksToBounds = YES;
    view.layer.cornerRadius = kCornerRadius;
...
// 添加提示语
- (void)addTipsLabel {
    _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake((kScreenW - _tipsSize.width) / 2, self.center.y + 20, _tipsSize.width, _tipsSize.height)];
...
- (void)initLoadingAttribute {
    if (_popViewType == MHPopViewTypeWrapContentWithTips || _popViewType == MHPopViewTypeFullScreenWithTips) {
        _tipsArray = @[JOIN1(kTips),
                       ...
                       JOIN2(kTips, ......)];
        _tipsIndex = 0;
        _tipsSize = [self getSizeOfString:_tipsArray[_tipsArray.count - 1] maxBoundSizeWidth:kScreenW - 100 fontSize:17];
    }
}
...
- (void)showTips:(NSString *)tips
    ...
    CGSize size = [self getSizeOfString:tips
                      maxBoundSizeWidth:kScreenW - 40
                               fontSize:17];
    _tipsLabel = [[UILabel alloc] initWithFrame:
                  CGRectMake((kScreenW - size.width - 30) / 2,
                             kScreenH - 150,
                             size.width + 20,
                             size.height)];
...
- (CGSize)getSizeOfString:(NSString*)string
    ...
    CGSize size = [string
                   boundingRectWithSize:CGSizeMake(width, kScreenH)
