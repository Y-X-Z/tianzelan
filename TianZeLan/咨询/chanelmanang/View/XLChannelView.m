...
-(void)buildUI
{
    ...
    _inUseTitleView.title = @"我的频道";
    _inUseTitleView.subTitle = @"单击增删频道 长按拖拽排序";
    ...
    _unUseTitleView.title = @"更多频道";
    _unUseTitleView.subTitle = @"点击添加频道";
...
-(void)itemTapMethod:(UIGestureRecognizer*)gesture
{
    ...
    if ([_inUseItems containsObject:item]) {
        NSLog(@"这是上面的");
        //只有一个的时候不能删除
        //if (_inUseItems.count == 4) {return;}
        //[_inUseItems removeObject:item];
        //[_unUseItems insertObject:item atIndex:0];
        if ([item.title isEqualToString:@"推荐"]) {return;}
        if ([item.title isEqualToString:@"本地"]) {return;}
        if ([item.title isEqualToString:@"视频"]) {return;}
        if ([item.title isEqualToString:@"体育"]) {return;}
        [_inUseItems removeObject:item];
        [_unUseItems insertObject:item atIndex:0];
    }else{
        NSLog(@"这是下面的");
