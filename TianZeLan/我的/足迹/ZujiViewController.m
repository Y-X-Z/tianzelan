...
@interface ZujiViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
{
...
@implementation ZujiViewController
...
-(void)inittabview{
    ...
    allbtn = [YX_myUI createButtonWithFrame:CGRectMake(10, 10, 60, 30) Target:self Selector:@selector(alltime) Image:@"ALL" ImagePressed:@"ALL"];
    
    timelabel = [YX_myUI createLabelWithFrame:CGRectMake(kScreenW-110, 10, 100, 30) Font:14 Text:@"05月20日" textcolor:[UIColor blackColor] TextAlignment:NSTextAlignmentCenter numberOfLines:1];
    
    CGFloat width = kScreenW - 20.0;
    CGPoint origin = CGPointMake(10.0, 50.0);
    
    calendar = [[GFCalendarView alloc] initWithFrameOrigin:origin width:width];
    
    headerview=[YX_myUI createViewWithFrame:CGRectMake(0, 0, kScreenW, calendar.height) Backgroundcolor:RGBA(242, 242, 242, 1)];
    ...
    calendar.didSelectDayHandler = ^(NSInteger year, NSInteger month, NSInteger day) {
        NSLog(@"year,month,day====%ld%ld%ld",year,month,day);
        flow.headerReferenceSize =CGSizeMake(kScreenW,50);//头视图大小
        calendar.hidden=YES;
        timelabel.text=[NSString stringWithFormat:@"%ld月%ld日",month,day];
    };
    ...
    flow.headerReferenceSize =CGSizeMake(kScreenW,calendar.height+60);//头视图大小
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.titlelabel.text = [NSString stringWithFormat:@"%@",titlearr[arc4random()%6]];
    [cell.headimagev sd_setImageWithURL:[NSURL URLWithString:arr[arc4random()%6]] placeholderImage:[UIImage imageNamed:@"no_image.9"]];
    return cell;
}
